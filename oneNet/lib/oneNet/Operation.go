...
const (
	TimeFormat     = "2006-01-02 15DeviceAdd2:04:05"
...
func (on *OneNet) Device(id int) (bool, *string) {
	...
	api := "mqtt/v1/devices/" + strconv.Itoa(id) + "/image"
	headers := make(map[string]string)
	headers["Authorization"] = "version=2018-10-31&res=mqtt%2Fv1%2Fdevices%2F599079417%2Fimage&et=1590574517&method=sha1&sign=dedjEcaS%2FXMORAvlHN0LlUR%2BzSQ%3D"
	return on.call(&api, ALLOW_METHODS["GET"], nil, headers)
}
func (on *OneNet) Device2(id int) (bool, *string) {
	api := "/devices/" + strconv.Itoa(id)
...
func (on *OneNet) DeviceAdd2(device interface{}) string {
	api := "mqtt/v1/devices/reg"

	headers := make(map[string]string)
	headers["Authorization"] = "version=2018-10-31&res=products%2F347322&et=1622109008&method=md5&sign=eMdokJpLpDBpDXKi2%2FWTNQ%3D%3D"
	//return on.call(&api, ALLOW_METHODS["POST"], device, nil)
	return on.SendPostWithHeard(&api,  device, headers)
}

func (on *OneNet) DeviceAdd3(device interface{}) (bool, *string) {
	api := "mqtt/v1/devices/reg"
	headers := make(map[string]string)
	headers["Authorization"] = "version=2018-10-31&res=products%2F347322&et=1622109008&method=md5&sign=eMdokJpLpDBpDXKi2%2FWTNQ%3D%3D"
	//return on.call(&api, ALLOW_METHODS["POST"], device, nil)
	return on.call(&api, ALLOW_METHODS["POST"], device, headers)
}
